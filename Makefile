...
L10N_LANGS := fr uk
L10N_TESTS := -only-testing:RationTests/LocalizationProofTests -only-testing:RationTests/CatalogCompletenessTests -only-testing:RationTests/CatalogChecksTests -only-testing:RationTests/AppLanguageTests -only-testing:RationTests/ThemeFontTests -only-testing:RationTests/FormattersLocalizationTests -only-testing:RationTests/SwitchAdviceCopyLocalizationTests -only-testing:RationTests/AlertMessageLocalizationTests -only-testing:RationTests/BannerCopyLocalizationTests -only-testing:RationTests/ResetCopyLocalizationTests -only-testing:RationTests/FocusAndDropCopyLocalizationTests -only-testing:RationTests/LocalizedCopyTests -only-testing:RationTests/SettingsAndErrorCopyLocalizationTests -only-testing:RationTests/PersistedStateLocalizationTests -only-testing:RationTests/PopoverCopyLocalizationTests -only-testing:RationTests/SettingsViewCopyLocalizationTests -only-testing:RationTests/LanguagePickerModelTests -only-testing:RationTests/OnboardingAndAccountsCopyLocalizationTests -only-testing:RationTests/HistoryCopyLocalizationTests -only-testing:RationTests/LayoutFitCopyLocalizationTests -only-testing:RationTests/DueStateCopyLocalizationTests -only-testing:RationTests/LocalizedLayoutSnapshotTests
